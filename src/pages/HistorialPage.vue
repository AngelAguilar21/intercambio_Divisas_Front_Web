        ...
        <q-card flat bordered class="xc-card-accent historial-card">
          ...
          <div class="tabla-scroll">
            ...
            <table v-else class="historial-table">
              ...
              <tbody>
                <template v-for="(fila, idx) in cols[colCfg.key].filas" :key="filaKey(fila, idx)">
                  <tr>
...
// Contrato de API (parámetros `tipo`, respuesta `data.registros`/`data.totalPaginas`,
// nombres de campo) tomado de master, que es el que quedó alineado con el backend Supabase.
...
const COLUMNAS_CONFIG = [
  {
    ...
    expandible: false,
    campos: [
      { key: 'fechaCreacion', label: 'Fecha', fn: (r) => fmt(r.fechaCreacion) },
      { key: 'monedaOrigen', label: 'Origen' },
      { key: 'monedaDestino', label: 'Destino' },
      { key: 'cantidadOriginal', label: 'Original' },
      { key: 'cantidadPendiente', label: 'Pendiente' },
  ...
  },
  {
    ...
    campos: [
      ...
      { key: 'cantidadObtenida', label: 'Cantidad' },
...
function filaKey(fila, idx) {
  return fila.ordenCompraId || fila.ofertaVentaId || fila.operacionInmediataId || idx
}
  ...
  await Promise.all(
    COLUMNAS_CONFIG.map(async (colCfg) => {
      ...
      try {
        ...
        resultado[colCfg.key] = data.registros || []
      } catch {
...
async function exportarPdf() {
  ...
  try {
    ...
    ventana.document.write(`
      <html><head><title>Historial de operaciones</title>
      <style>
