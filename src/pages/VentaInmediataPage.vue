  ...
  <q-page class="q-pa-md">
    ...
    <q-card flat bordered class="q-pa-md q-mb-md">
      <div class="row q-col-gutter-md items-end">
        ...
        <div class="col-12 col-md-3">
          <q-input v-model.number="form.cantidad" type="number" label="Cantidad a vender" outlined />
        </div>
        <div class="col-12 col-md-3">
    ...
    <q-card v-if="resumen" flat bordered class="q-pa-md q-mb-md">
      ...
      <q-list dense>
        <q-item>
          ...
          <q-item-section side>{{ resumen.cantidadEjecutable }} {{ resumen.monedaOrigen }}</q-item-section>
        </q-item>
        <q-item>
        ...
        <q-item>
          ...
          <q-item-section side>{{ resumen.totalEstimadoARecibir }} {{ resumen.monedaDestino }}</q-item-section>
        </q-item>
      </q-list>
    ...
    <q-card v-if="resultado" flat bordered class="q-pa-md bg-green-1">
      ...
      <q-list dense>
        <q-item>
          ...
          <q-item-section side>{{ resultado.cantidadEjecutada }} {{ resultado.monedaOrigen }}</q-item-section>
        </q-item>
        <q-item>
          <q-item-section>Total recibido</q-item-section>
          <q-item-section side>{{ resultado.totalRecibido }} {{ resultado.monedaDestino }}</q-item-section>
        </q-item>
        <q-item>
...
async function onVerResumen() {
  ...
  try {
    ...
    const { data } = await getResumen({ parMonedaId: parMonedaId.value, cantidadAVender: form.cantidad })
    resumen.value = data
  } catch (error) {
